...
public class MySQLSinkPlugin implements SinkPlugin {
    ...
    public SinkFunction<?> createSink(SinkConfig config) {
        ...
        String columnsStr = config.getString("columns");
        ...
        if (url == null || username == null || password == null || table == null || columnsStr == null) {
            throw new IllegalArgumentException("MySQL Sink 缺少必要配置: url, username, password, table, columns");
        }
        ...
        return new MySQLSinkFunction(url, username, password, table, columns, batchSize, writeMode);
    }
}
